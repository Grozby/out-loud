...
public class PaletteBuilder {
   ...
   protected PaletteBuilder.ColorNode insertNode(PaletteBuilder.ColorNode aNode, Color aColor, int aLevel) {
      if (aNode == null) {
         aNode = new PaletteBuilder.ColorNode();
         this.numNodes++;
      ...
      aNode.colorCount++;
      aNode.red = aNode.red + (long)aColor.getRed();
      aNode.green = aNode.green + (long)aColor.getGreen();
      aNode.blue = aNode.blue + (long)aColor.getBlue();
      if (!aNode.isLeaf) {
   ...
   protected IndexColorModel getIndexColorModel() {
      ...
      int index = 0;
      this.palette = new PaletteBuilder.ColorNode[size];
      if (this.transparency != 1) {
         index++;
      }
   ...
   protected int getBranchIndex(Color aColor, int aLevel) {
      if (aLevel <= 8 && aLevel >= 0) {
         ...
         return red_index << 2 | green_index << 1 | blue_index;
      } else {
   ...
   protected void reduceTree() {
      int level = this.reduceList.length - 1;

      while (this.reduceList[level] == null && level >= 0) {
         level--;
      }

      PaletteBuilder.ColorNode thisNode = this.reduceList[level];
      if (thisNode != null) {
         PaletteBuilder.ColorNode pList = thisNode;
   ...
   protected class ColorNode {
      ...
      public ColorNode() {
         this.childCount = 0;
         this.children = new PaletteBuilder.ColorNode[8];
