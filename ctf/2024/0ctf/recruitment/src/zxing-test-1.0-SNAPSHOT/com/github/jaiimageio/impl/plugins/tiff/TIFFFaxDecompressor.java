...
public class TIFFFaxDecompressor extends TIFFDecompressor {
   ...
   static short[] additionalMakeup = new short[]{
      28679, 28679, 31752, -32759, -31735, -30711, -29687, -28663, 29703, 29703, 30727, 30727, -27639, -26615, -25591, -24567
   };
   ...
   public void decodeRLE() throws IIOException {
      for (int i = 0; i < this.h; i++) {
         ...
         this.lineBitNum = this.lineBitNum + this.bitsPerScanline;
      }
   }
      ...
      while (bitOffset < this.w) {
         ...
         while (isWhite && bitOffset < this.w) {
            ...
            if (bits == 12) {
               ...
               int var33 = additionalMakeup[current];
               bits = var33 >>> 1 & 7;
               code = var33 >>> 4 & 4095;
               bitOffset += code;
         ...
         while (!isWhite && bitOffset < this.w) {
            ...
            if (code == 100) {
               ...
               if (bits == 12) {
                  ...
                  var35 = additionalMakeup[current];
                  bits = var35 >>> 1 & 7;
                  code = var35 >>> 4 & 4095;
   ...
   public void decodeT4() throws IIOException {
      ...
      if (this.data.length < 2) {
         ...
         for (this.lineBitNum = this.lineBitNum + this.bitsPerScanline; lines < height; lines++) {
            try {
               modeFlag = this.findNextLine();
               ...
               while (bitOffset < this.w) {
                  ...
                  if (code == 0) {
                     ...
                     if (isWhite) {
                        ...
                        int var30 = bitOffset + number;
                        this.currChangingElems[currIndex++] = var30;
                        number = this.decodeBlackCodeWord();
                        this.setToBlack(var30, number);
                        bitOffset = var30 + number;
                        this.currChangingElems[currIndex++] = bitOffset;
                     } else {
                        int number = this.decodeBlackCodeWord();
                        this.setToBlack(bitOffset, number);
                        int var31 = bitOffset + number;
                        this.currChangingElems[currIndex++] = var31;
                        number = this.decodeWhiteCodeWord();
                        bitOffset = var31 + number;
                        this.currChangingElems[currIndex++] = bitOffset;
                     }
                     ...
                     if (code > 8) {
                        this.warning(
                           "Unknown coding mode encountered at line " + (this.srcMinY + lines) + ": read " + bitOffset + " of " + this.w + " expected pixels."
                        );
         ...
         while (bitOffset < this.w) {
            ...
            if (code == 0) {
               ...
               if (isWhite) {
                  ...
                  this.setToBlack(bitOffset, number);
                  int var27 = bitOffset + number;
                  cce[currIndex++] = var27;
                  number = this.decodeWhiteCodeWord();
                  bitOffset = var27 + number;
                  cce[currIndex++] = bitOffset;
               }
               ...
               int entranceCode = this.nextLesserThan8Bits(3);
               if (entranceCode != 7) {
   ...
   private void setToBlack(int bitNum, int numBits) {
      ...
      for (int var9 = bitNum >> 3; bitNum < lastBit - 7; bitNum += 8) {
         this.buffer[var9++] = -1;
      }

      while (bitNum < lastBit) {
         byteNum = bitNum >> 3;
         this.buffer[byteNum] = (byte)(this.buffer[byteNum] | 1 << 7 - (bitNum & 7));
         bitNum++;
      }
   }
   ...
   private int decodeWhiteCodeWord() throws IIOException {
      ...
      while (isWhite) {
         ...
         if (bits == 12) {
            int twoBits = this.nextLesserThan8Bits(2);
            current = current << 2 & 12 | twoBits;
            int var10 = additionalMakeup[current];
            bits = var10 >>> 1 & 7;
            code = var10 >>> 4 & 4095;
            runLength += code;
            this.updatePointer(4 - bits);
      ...
      while (!isWhite) {
         ...
         if (code == 100) {
            ...
            if (bits == 12) {
               this.updatePointer(5);
               current = this.nextLesserThan8Bits(4);
               var12 = additionalMakeup[current];
               bits = var12 >>> 1 & 7;
               code = var12 >>> 4 & 4095;
               runLength += code;
               this.updatePointer(4 - bits);
