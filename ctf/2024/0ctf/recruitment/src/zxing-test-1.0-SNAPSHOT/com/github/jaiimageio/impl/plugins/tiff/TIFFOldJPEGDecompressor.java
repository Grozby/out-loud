...
public class TIFFOldJPEGDecompressor extends TIFFJPEGDecompressor {
   ...
   private synchronized void initialize() throws IOException {
      if (!this.isInitialized) {
         ...
         if (JPEGInterchangeFormatField != null) {
            long jpegInterchangeOffsetx = JPEGInterchangeFormatField.getAsLong(0);
            TIFFField JPEGInterchangeFormatLengthFieldx = tim.getTIFFField(514);
            if (JPEGInterchangeFormatLengthFieldx != null) {
               long jpegInterchangeLength = JPEGInterchangeFormatLengthFieldx.getAsLong(0);
               if (jpegInterchangeLength >= 2L && jpegInterchangeOffsetx + jpegInterchangeLength <= segmentOffsets[0]) {
                  this.stream.mark();
                  this.stream.seek(jpegInterchangeOffsetx + jpegInterchangeLength - 2L);
                  ...
                  this.stream.reset();
                  this.stream.mark();
                  this.stream.seek(jpegInterchangeOffsetx);
                  this.stream.readFully(this.tables);
                  this.stream.reset();
                  ((TIFFImageReader)this.reader)
                     .forwardWarningMessage("Incorrect JPEG interchange format: using JPEGInterchangeFormat offset to derive tables.");
               } else {
         ...
         if (this.tables == null) {
            ...
            for (int i = 0; i < off.length; i++) {
               ...
               baos.write(219);
               char markerLength = 'C';
               baos.write(markerLength >>> '\b' & 0xFF);
            ...
            if (f != null) {
               ...
               if (restartInterval != 0) {
                  ...
                  baos.write(-35);
                  short var68 = 4;
                  baos.write((byte)(var68 >>> 8 & 0xFF));
                  baos.write((byte)(var68 & 255));
                  baos.write((byte)(restartInterval >>> '\b' & 0xFF));
         ...
         if (this.SOFPosition == -1) {
            ...
            short svalx = (short)(8 + 3 * this.samplesPerPixel);
            this.tables[tmpOffset++] = (byte)(svalx >>> 8 & 0xFF);
            this.tables[tmpOffset++] = (byte)(svalx & 255);
            this.tables[tmpOffset++] = 8;
            svalx = (short)this.srcHeight;
            this.tables[tmpOffset++] = (byte)(svalx >>> 8 & 0xFF);
            this.tables[tmpOffset++] = (byte)(svalx & 255);
            svalx = (short)this.srcWidth;
            this.tables[tmpOffset++] = (byte)(svalx >>> 8 & 0xFF);
            this.tables[tmpOffset++] = (byte)(svalx & 255);
            ...
            if (this.samplesPerPixel == 1) {
               ...
               for (int i = 0; i < 3; i++) {
                  this.tables[tmpOffset++] = (byte)(i + 1);
                  this.tables[tmpOffset++] = i != 0 ? 17 : (byte)((this.subsamplingX & 15) << 4 | this.subsamplingY & 15);
                  this.tables[tmpOffset++] = (byte)i;
               }
            }
         }
         ...
         if (this.stream.read() == 255 && this.stream.read() == 218) {
            ...
            short svalx = (short)(6 + 2 * this.samplesPerPixel);
            this.SOSMarker[SOSMarkerIndex++] = (byte)(svalx >>> 8 & 0xFF);
            this.SOSMarker[SOSMarkerIndex++] = (byte)(svalx & 255);
