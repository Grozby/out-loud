...
public class GIFImageWriter extends ImageWriter {
   ...
   private void writeImage(
      RenderedImage image, GIFWritableImageMetadata imageMetadata, ImageWriteParam param, byte[] globalColorTable, Rectangle sourceBounds, Dimension destSize
   ) throws IOException {
      ColorModel colorModel = image.getColorModel();
      SampleModel sampleModel = image.getSampleModel();
      ...
      int bitsPerPixel = getNumBits(
         imageMetadata.localColorTable == null
            ? (globalColorTable == null ? sampleModel.getSampleSize(0) : globalColorTable.length / 3)
   ...
   private void writeRowsOpt(
      byte[] data, int offset, int lineStride, LZWCompressor compressor, int dy, int ddy, int dw, int dh, int numRowsWritten, int progressReportRowPeriod
   ) throws IOException {
   ...
   private void writeRasterData(RenderedImage image, Rectangle sourceBounds, Dimension destSize, ImageWriteParam param, boolean interlaceFlag) throws IOException {
      ...
      boolean isOptimizedCase = periodX == 1
         && periodY == 1
      ...
      if (interlaceFlag) {
         if (isOptimizedCase) {
            ...
            int offset = csm.getOffset(sourceXOffset - tile.getSampleModelTranslateX(), sourceYOffset - tile.getSampleModelTranslateY(), 0);
         ...
         Raster tilex = image.getTile(0, 0);
         byte[] datax = ((DataBufferByte)tilex.getDataBuffer()).getData();
         ComponentSampleModel csmx = (ComponentSampleModel)tilex.getSampleModel();
         int offsetx = csmx.getOffset(sourceXOffset - tilex.getSampleModelTranslateX(), sourceYOffset - tilex.getSampleModelTranslateY(), 0);
         int lineStridex = csmx.getScanlineStride();
         this.writeRowsOpt(datax, offsetx, lineStridex, compressor, 0, 1, destWidth, destHeight, numRowsWritten, progressReportRowPeriod);
      } else {
         this.writeRows(
            image,
            compressor,
