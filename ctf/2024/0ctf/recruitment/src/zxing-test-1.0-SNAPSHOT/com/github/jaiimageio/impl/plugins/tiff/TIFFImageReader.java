...
public class TIFFImageReader extends ImageReader {
         ...
         if (planarConfigurationValue == 2) {
            if (this.getCompression() == 6 && this.imageMetadata.getTIFFField(513) != null) {
               ...
               if (offsetField == null) {
                  ...
                  int tw = this.getTileOrStripWidth();
                  int th = this.getTileOrStripHeight();
                  int tAcross = (this.getWidth() + tw - 1) / tw;
                  int tDown = (this.getHeight() + th - 1) / th;
                  ...
                  if (offsetArray != null && offsetArray.length == tilesPerImage) {
                     this.processWarningOccurred(
                        "PlanarConfiguration \"Planar\" value inconsistent with TileOffsets field value count; resetting to \"Chunky\"."
                     );
                     planarConfigurationValue = 1;
                  }
               } else {
                  int rowsPerStrip = this.getTileOrStripHeight();
                  int stripsPerImage = (this.getHeight() + rowsPerStrip - 1) / rowsPerStrip;
                  long[] offsetArray = offsetField.getAsLongs();
   ...
   private long getTileOrStripByteCount(int tileIndex) throws IOException {
      ...
      if (f != null) {
         ...
         int bytesPerRow = (this.getTileOrStripWidth() * bitsPerPixel + 7) / 8;
   ...
   private void initializeFromMetadata() {
      ...
      for (int ix = 0; ix < this.samplesPerPixel; ix++) {
         this.bitsPerSample[ix] = replicateFirst ? first : f.getAsInt(ix);
      }
   ...
   public Iterator getImageTypes(int imageIndex) throws IIOException {
      ...
      if (this.imageTypeMap.containsKey(imageIndexInteger)) {
         ...
         l = new ArrayList(1);
         this.seekToImage(imageIndex);
         ImageTypeSpecifier itsRaw = TIFFDecompressor.getRawImageTypeSpecifier(
         ...
         if (iccProfileField != null && itsRaw.getColorModel() instanceof ComponentColorModel) {
            ...
            if (numBands != numComponents && numBands != numComponents + 1) {
               ...
               ColorModel iccColorModel = new ComponentColorModel(
                  iccColorSpace, cmRaw.getComponentSize(), hasAlpha, isAlphaPre, cmRaw.getTransparency(), cmRaw.getTransferType()
               );
   ...
   private void decodeTile(int ti, int tj, int band) throws IOException {
      ...
      if (tileRect.width > 0 && tileRect.height > 0) {
         ...
         this.dstMinX = this.dstMinX + this.dstXOffset;
         this.dstMinY = this.dstMinY + this.dstYOffset;
   ...
   private void reportProgress() {
      this.pixelsRead = this.pixelsRead + this.dstWidth * this.dstHeight;
   ...
   public BufferedImage read(int imageIndex, ImageReadParam param) throws IOException {
      ...
      if (this.decompressor == null) {
         if (compression == 1) {
            ...
            TIFFField predictorField = this.imageMetadata.getTIFFField(317);
            int predictor = predictorField == null ? 1 : predictorField.getAsInt(0);
