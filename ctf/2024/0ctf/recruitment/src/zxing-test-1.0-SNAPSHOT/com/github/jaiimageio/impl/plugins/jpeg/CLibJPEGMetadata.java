...
public class CLibJPEGMetadata extends IIOMetadata {
   ...
   static final int[] zigzag = new int[]{
      0,
      1,
      5,
      6,
   ...
   private static IIOImage getThumbnail(ImageInputStream stream, int len, int thumbnailType, int w, int h) throws IOException {
      ...
      if (thumbnailType == 16) {
         ...
         WritableRaster wr = Raster.createInterleavedRaster(db, w, h, w * numBands, numBands, new int[]{0, 1, 2}, null);
   ...
   CLibJPEGMetadata() {
      super(
         ...
         new String[]{"com_sun_media_imageio_plugins_tiff_image_1.0"},
         new String[]{"com.github.jaiimageio.impl.plugins.tiff.TIFFImageMetadataFormat"}
      );
            ...
            if (iis.read() == 255) {
               ...
               if (code != 0 && code != 255 && code != 216 && code != 1 && (code < 208 || code > 215)) {
                  ...
                  if (224 <= code && code <= 239) {
                        ...
                        case 224:
                           if (dataLength >= 5) {
                              ...
                              if (id.startsWith("JFIF") && !this.app0JFIFPresent) {
                                 ...
                                 this.extensionCodes.add(new Integer(extCode));
                                 int w = 0;
                                 int h = 0;
                                 int offset = 6;
                                 if (extCode != 16) {
                           ...
                           if (dataLength >= 12) {
                              ...
                              if (id.startsWith("ICC_PROFILE")) {
                                 ...
                                 if (numChunks == 0
                                    || chunkNum == 0
                                    || chunkNum > numChunks
                                    || this.app2ICCPresent && (numChunks != numICCProfileChunks || iccProfileChunkOffsets[chunkNum] != 0L)) {
                     ...
                     do {
                        CLibJPEGMetadata.QTable t = new CLibJPEGMetadata.QTable(iis);
                        l.add(t);
                        dataLength -= t.length;
                     } while (dataLength > 0);
                     ...
                     do {
                        CLibJPEGMetadata.HuffmanTable t = new CLibJPEGMetadata.HuffmanTable(iis);
                        l.add(t);
                        dataLength -= t.length;
                     } while (dataLength > 0);
   ...
   private Node getNativeTree() {
      ...
      for (int marker : this.markers) {
         switch (marker) {
            case 196:
               IIOMetadataNode dht = new IIOMetadataNode("dht");
               List tables = (List)this.htables.get(dhtIndex++);
               int numTables = tables.size();

               for (int j = 0; j < numTables; j++) {
                  IIOMetadataNode dhtable = new IIOMetadataNode("dhtable");
                  CLibJPEGMetadata.HuffmanTable t = (CLibJPEGMetadata.HuffmanTable)tables.get(j);
            ...
            case 219:
               ...
               List tables = (List)this.qtables.get(dqtIndex++);
               int numTables = tables.size();

               for (int j = 0; j < numTables; j++) {
                  IIOMetadataNode dqtable = new IIOMetadataNode("dqtable");
                  CLibJPEGMetadata.QTable t = (CLibJPEGMetadata.QTable)tables.get(j);
            ...
            case 221:
               ...
               dri.setAttribute("interval", Integer.toString(this.driInterval));
               markerSequence.appendChild(dri);
               break;
            case 254:
               IIOMetadataNode com = new IIOMetadataNode("com");
               com.setUserObject(this.comments.get(comIndex++));
         ...
         if (this.app0JFIFPresent) {
            ...
            if (this.numFrameComponents < 3) {
               ...
               for (int i = 0; i < this.componentId.length; i++) {
                  int id = this.componentId[i];
                  if (id < 1 || id >= this.componentId.length) {
                     idsAreJFIF = false;
                  }
               }
               ...
               if (idsAreJFIF) {
                  ...
                  int hfactor = this.hSamplingFactor[0];
                  int vfactor = this.vSamplingFactor[0];

                  for (int ix = 1; ix < this.componentId.length; ix++) {
                     if (this.hSamplingFactor[ix] != hfactor || this.vSamplingFactor[ix] != vfactor) {
                        subsampled = true;
                        break;
                     }
                  }
   ...
   private Node getTIFFTree() {
      ...
      if (this.sofPresent) {
         int compression = 7;
         TIFFField compressionField = new TIFFField(base.getTag(259), compression);
         dir.addTIFFField(compressionField);
         char[] bitsPerSample = new char[this.numFrameComponents];
         Arrays.fill(bitsPerSample, (char)(this.samplePrecision & 0xFF));
         TIFFField bitsPerSampleField = new TIFFField(base.getTag(258), 3, bitsPerSample.length, bitsPerSample);
         ...
         TIFFField imageWidthField = new TIFFField(base.getTag(256), this.samplesPerLine);
         ...
         if (chroma != null) {
            ...
            if (photometricInterpretation != -1) {
               TIFFField photometricInterpretationField = new TIFFField(base.getTag(262), photometricInterpretation);
      ...
      if (this.app0JFIFPresent) {
         ...
         TIFFField XResolutionField = new TIFFField(base.getTag(282), 5, 1, xResolution);
         dir.addTIFFField(XResolutionField);
         long[][] yResolution = new long[][]{{(long)this.Ydensity, 1L}};
         TIFFField YResolutionField = new TIFFField(base.getTag(283), 5, 1, yResolution);
         ...
         TIFFField ResolutionUnitField = new TIFFField(base.getTag(296), resolutionUnit);
      ...
      if (this.dqtPresent || this.dqtPresent) {
         ...
         if (this.dqtPresent) {
            Iterator dqts = this.qtables.iterator();

            while (dqts.hasNext()) {
               for (CLibJPEGMetadata.QTable qt : (List)dqts.next()) {
         ...
         if (this.dhtPresent) {
            Iterator dhts = this.htables.iterator();

            while (dhts.hasNext()) {
               for (CLibJPEGMetadata.HuffmanTable ht : (List)dhts.next()) {
         ...
         if (this.dqtPresent) {
            ...
            while (dqts.hasNext()) {
               ...
               while (qtiter.hasNext()) {
                  ...
                  CLibJPEGMetadata.QTable qt = (CLibJPEGMetadata.QTable)qtiter.next();
         ...
         if (this.dhtPresent) {
            ...
            while (dhts.hasNext()) {
               ...
               while (htiter.hasNext()) {
                  ...
                  CLibJPEGMetadata.HuffmanTable ht = (CLibJPEGMetadata.HuffmanTable)htiter.next();
      ...
      if (jpegTablesData != null) {
         TIFFField JPEGTablesField = new TIFFField(base.getTag(347), 7, jpegTablesData.length, jpegTablesData);
