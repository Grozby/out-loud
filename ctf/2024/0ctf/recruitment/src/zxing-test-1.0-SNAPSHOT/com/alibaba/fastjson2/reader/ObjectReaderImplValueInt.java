...
public class ObjectReaderImplValueInt<T> implements ObjectReader<T> {
   ...
   public T readObject(JSONReader jsonReader, Type fieldType, Object fieldName, long features) {
      if (jsonReader.nextIfNullOrEmptyString()) {
         ...
         if (this.schema != null) {
            this.schema.validate((long)value);
         }
   ...
   public static <T> ObjectReaderImplValueInt<T> of(Class<T> objectClass, IntFunction<T> function) {
      return new ObjectReaderImplValueInt<>(objectClass, 0L, null, function);
   }

   public static <T> ObjectReaderImplValueInt<T> of(Class<T> objectClass, long features, JSONSchema schema, IntFunction<T> function) {
      return new ObjectReaderImplValueInt<>(objectClass, features, schema, function);
   }
}
