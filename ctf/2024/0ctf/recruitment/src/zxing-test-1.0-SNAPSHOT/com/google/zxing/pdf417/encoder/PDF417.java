...
public final class PDF417 {
   ...
   private static final int[][] CODEWORD_TABLE = new int[][]{
      {
            120256,
            125680,
            128380,
   ...
   public void generateBarcodeLogic(String msg, int errorCorrectionLevel) throws WriterException {
      this.generateBarcodeLogic(msg, errorCorrectionLevel, false);
   }
   ...
   public void generateBarcodeLogic(String msg, int errorCorrectionLevel, boolean autoECI) throws WriterException {
      ...
      String highLevel = PDF417HighLevelEncoder.encodeHighLevel(msg, this.compaction, this.encoding, autoECI);
      int sourceCodeWords = highLevel.length();
      int[] dimension = this.determineDimensions(sourceCodeWords, errorCorrectionCodeWords);
      ...
      if (sourceCodeWords + errorCorrectionCodeWords + 1 > 929) {
         ...
         this.encodeLowLevel(dataCodewords + ec, cols, rows, errorCorrectionLevel, this.barcodeMatrix);
      }
   }
   ...
   private int[] determineDimensions(int sourceCodeWords, int errorCorrectionCodeWords) throws WriterException {
      ...
      for (int cols = this.minCols; cols <= this.maxCols; cols++) {
         ...
         if (rows <= this.maxRows) {
            float newRatio = (float)(17 * cols + 69) * 0.357F / ((float)rows * 2.0F);
            if (dimension == null || !(Math.abs(newRatio - 3.0F) > Math.abs(ratio - 3.0F))) {
               ratio = newRatio;
      ...
      if (dimension == null) {
         int rowsx = calculateNumberOfRows(sourceCodeWords, errorCorrectionCodeWords, this.minCols);
         if (rowsx < this.minRows) {
