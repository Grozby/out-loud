...
final class DecoderConfig {
   ...
   Map<DecodeHintType, ?> buildHints() {
      List<BarcodeFormat> finalPossibleFormats = this.possibleFormats;
      if (finalPossibleFormats == null || finalPossibleFormats.isEmpty()) {
         finalPossibleFormats = new ArrayList<>(
            Arrays.asList(BarcodeFormat.UPC_A, BarcodeFormat.UPC_E, BarcodeFormat.EAN_13, BarcodeFormat.EAN_8, BarcodeFormat.RSS_14, BarcodeFormat.RSS_EXPANDED)
         );
         if (!this.productsOnly) {
            finalPossibleFormats.addAll(
               Arrays.asList(
